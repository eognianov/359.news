namespace NewsSystem.Data.Seeding
{
    ...
    public class WorkerTasksSeeder : ISeeder
    {
        public void Seed(ApplicationDbContext dbContext, IServiceProvider serviceProvider)
        {
            var workerTasks = new List<WorkerTask>
                              {
                                  new WorkerTask
                                  {
                                      TypeName = "NewsSystem.Worker.Tasks.DbCleanupTask",
                                      Parameters = "{\"Recreate\":true}",
                                      Priority = 0,
                                  },
                                  new WorkerTask
                                  {
                                      TypeName = "NewsSystem.Worker.Tasks.NewsTagsTask",
                                      Parameters = "{\"Recreate\":true,\"LastId\":0}",
                                      Priority = 2000,
                                  },
                                  new WorkerTask
                                  {
                                      TypeName = "NewsSystem.Worker.Tasks.MainNewsGetterTask",
                                      Parameters = "{\"Recreate\":true}",
                                      Priority = 10000,
                                  },
                              };
            ...
            // Sources workers
            const string LatestPublicationsTaskName = "NewsSystem.Worker.Tasks.GetLatestPublicationsTask";
