...
namespace NewsSystem.App.Areas.Administration.Controllers
{
    ...
    public class OfficeController : AdministrationController
    {
        ...
        public async Task<IActionResult> Published(string period)
        {
            ...
            {
                case "yestarday":
                    news = await this.newsRepository.AllPublished().Where(n => n.PublishedOn.Value.Date == yestarday).OrderByDescending(x => x.PublishedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    break;

                case "week":
                    news = await this.newsRepository.AllPublished().Where(n => n.PublishedOn.Value.Date <= yestarday && n.PublishedOn.Value.Date >= week).OrderByDescending(x => x.PublishedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    break;

                default:
                    news = await this.newsRepository.AllPublished().Where(n => n.PublishedOn.Value.Date == today).OrderByDescending(x => x.PublishedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    break;
            }
        ...
        public async Task<IActionResult> Deleted(string period)
        {
            ...
            {
                case "yestarday":
                    news = await this.newsRepository.AllWithDeleted().Where(n => n.IsDeleted == true && n.DeletedOn.Value.Date == yestarday).OrderByDescending(x => x.DeletedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    returnUrl += "?period=yestarday";
                ...
                case "week":
                    news = await this.newsRepository.AllWithDeleted().Where(n => n.IsDeleted == true && n.DeletedOn.Value.Date <= yestarday && n.DeletedOn.Value.Date >= week).OrderByDescending(x => x.DeletedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    returnUrl += "?period=week";
                ...
                default:
                    news = await this.newsRepository.AllWithDeleted().Where(n => n.IsDeleted == true && n.DeletedOn.Value.Date == today).OrderByDescending(x => x.DeletedOn)
                        .ThenByDescending(x => x.Id).To<NewsViewModel>().ToListAsync();
                    viewModel.News = news;
                    break;
            }
